...
const copy = computed(() => locale.value === "en" ? {
  ...
  eyebrow: "Living design system · 0.2.0",
  ...
  back: "العودة للتجارب",
  eyebrow: "نظام التصميم الحي · 0.2.0",
...
<template>
  <main :dir="direction" :lang="locale" class="min-h-screen bg-sharm-canvas px-5 py-6 text-sharm-ink lg:px-10">
    ...
    <section class="mx-auto max-w-7xl py-14">
      ...
      <h1 class="font-display mt-4 max-w-4xl text-4xl font-semibold tracking-tight sm:text-6xl">{{ copy.title }}</h1>
    ...
    <div class="mx-auto grid max-w-7xl gap-6 pb-16 lg:grid-cols-2">
      <section class="rounded-[1.75rem] border border-sharm-border bg-sharm-surface p-6">
        ...
        <div class="mt-5 grid gap-3 sm:grid-cols-2">
          <div v-for="swatch in swatches" :key="swatch.name" class="overflow-hidden rounded-2xl border border-sharm-border">
            ...
            <code class="reference block bg-sharm-surface p-3 text-sm">{{ swatch.value }}</code>
          </div>
        </div>
      </section>
      ...
      <section class="rounded-[1.75rem] border border-sharm-border bg-sharm-surface p-6">
        ...
        <p class="font-display mt-7 text-4xl leading-tight font-semibold">{{ copy.display }}</p>
