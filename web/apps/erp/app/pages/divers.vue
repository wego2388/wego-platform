...
import { WegoAlert, WegoBadge, WegoButton, WegoInput, WegoPageHeader, WegoPagination, WegoPanel, WegoSelect, WegoTextarea } from "@wego/ui";
...
async function startEdit(summary: DiverSummary) {
  ...
  formError.value = "";
  // The CSS reduced-motion guard (main.css) only overrides transition/
  // animation properties — it has no effect on a JS-driven scrollTo, so
  // that preference is checked explicitly here too.
...
<template>
  <WegoPageHeader title="Divers" description="Certifications, dive history, and equipment sizing for every customer profile." />
  ...
  <template v-else>
    ...
    <WegoPanel title="Diver profiles" class="mt-8">
      ...
      <template v-else>
        ...
        <ul v-else-if="divers.length > 0" class="mt-4 space-y-3">
          <li v-for="diver in divers" :key="diver.id" class="rounded-wego-control border border-wego-border p-4">
            <div class="flex flex-wrap items-start justify-between gap-3">
              <div>
                <div class="flex items-center gap-2">
                  <p class="font-semibold">{{ diver.fullName }}</p>
                  <WegoBadge :tone="diver.status === 'ACTIVE' ? 'success' : 'neutral'">{{ diver.status }}</WegoBadge>
                </div>
                ...
                <p class="mt-1 text-sm text-wego-muted">
                  ...
                  ><span v-if="diver.lastDiveOn"> · last dive {{ diver.lastDiveOn }}</span>
                </p>
                <p v-if="diver.certifications.length > 0" class="mt-2 flex flex-wrap gap-2">
                  <WegoBadge v-for="certification in diver.certifications" :key="`${certification.agency}-${certification.level}`" tone="accent">
    ...
    <WegoPanel v-if="canManage()" :title="editingDiverId ? 'Edit diver profile' : 'New diver profile'" class="mt-8">
      <form class="space-y-5" @submit.prevent="submitForm">
        ...
        <WegoTextarea
          ...
          help="Staff reference only — never a dive-fitness decision."
        />
