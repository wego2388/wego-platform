...
<template>
  <main class="min-h-screen bg-wego-canvas px-6 py-12 text-wego-ink sm:px-10 lg:px-16">
    <div class="mx-auto max-w-4xl">
      ...
      <h1 class="mt-4 text-3xl font-semibold tracking-tight">Financial Reports</h1>
      ...
      <template v-else>
        <section class="mt-8 rounded-wego-card border border-wego-border bg-wego-surface p-6">
          <h2 class="text-xl font-semibold">Trial Balance</h2>
          <p class="mt-2 text-sm text-wego-muted">Every account's balance as of a date — total debits should equal total credits.</p>
          <div class="mt-4 flex flex-wrap items-end gap-3">
            <WegoInput id="trialBalanceDate" v-model="trialBalanceDate" label="As of date" type="date" class="min-w-0 flex-1" />
            <WegoButton type="button" :disabled="!trialBalanceDate" @click="runTrialBalance">Run</WegoButton>
          </div>
        ...
        <section class="mt-8 rounded-wego-card border border-wego-border bg-wego-surface p-6">
          <h2 class="text-xl font-semibold">Income Statement</h2>
          <p class="mt-2 text-sm text-wego-muted">Revenue minus expenses over a period.</p>
          <div class="mt-4 flex flex-wrap items-end gap-3">
            ...
            <WegoButton type="button" :disabled="!incomeFrom || !incomeTo" @click="runIncomeStatement">Run</WegoButton>
          </div>
        ...
        <section class="mt-8 rounded-wego-card border border-wego-border bg-wego-surface p-6">
          ...
          <p class="mt-2 text-sm text-wego-muted">
            Assets, liabilities, and equity as of a date — assets should equal liabilities plus equity.
          </p>
          <div class="mt-4 flex flex-wrap items-end gap-3">
            <WegoInput id="balanceSheetDate" v-model="balanceSheetDate" label="As of date" type="date" class="min-w-0 flex-1" />
            <WegoButton type="button" :disabled="!balanceSheetDate" @click="runBalanceSheet">Run</WegoButton>
          </div>
