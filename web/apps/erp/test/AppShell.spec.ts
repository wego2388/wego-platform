...
describe("app-shell layout", () => {
  it("renders every nav group and link exactly once — no parallel desktop/mobile copy", async () => {
    ...
    for (const group of ["Overview", "Diving Operations", "People", "Finance", "Administration"]) {
      expect(wrapper.findAll("p").filter((p) => p.text() === group)).toHaveLength(1);
    }
    for (const link of ["Dashboard", "Offerings", "Bookings", "Chart of Accounts", "Staff Accounts"]) {
