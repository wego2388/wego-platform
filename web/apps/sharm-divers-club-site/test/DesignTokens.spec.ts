...
describe("Sharm Divers Club design-token contract", () => {
  it("keeps the repo design source aligned with executable semantic CSS", () => {
    const tokens = JSON.parse(readFileSync(resolve(process.cwd(), "../../../clients/sharm-divers-club/design/tokens.json"), "utf8")) as TokenDocument;
    const css = readFileSync(resolve(process.cwd(), "app/assets/css/main.css"), "utf8");
    const criticalTokens: Array<[string, string]> = [
      ["brand-deep", tokens.color.brand.deep],
      ["brand-turquoise", tokens.color.brand.turquoise],
    ...
    for (const [name, value] of criticalTokens) {
      expect(css).toContain(`--sdc-color-${name}: ${value};`);
    }
  });
