...
- **What CI's first real run found, and what was fixed:**
  ...
  4. **`infrastructure` job — a Playwright flake, confirmed genuinely environmental, not a logic bug, across two real CI runs.** Round 1 (this packet's first push): `create offering` (the pre-existing, untouched WEGO-002 booking test) timed out waiting for its own offering title to appear after form submission. Round 2 (the very next push, fixing the gitleaks self-quote below): a *different* test, the new financial-reports lifecycle test, timed out on its own login step's `getByText('Signed in as ...')` — an assertion that had already passed identically in 4 other tests earlier in that same run. Two occurrences, two unrelated tests, two unrelated post-action UI-update assertions, both simple 5000ms `expect()` timeouts — never reproduced locally or in Codex's own review sandbox, only on GitHub's shared `ubuntu-24.04` runner. This pattern (different step, different test, same "an already-proven-working assertion occasionally needs slightly more than 5s") is the signature of a real but ordinary CI-compute timing margin, not a deterministic defect in either test. **Fixed**: doubled `e2e/playwright.config.ts`'s `expect` timeout to 10s (with a comment recording exactly this evidence) — a targeted response to the measured symptom, not a blind retry that could mask a real future bug.
...
- **Evidence:** `bash scripts/repository-check.sh` — clean, locally, after the status-string fixes. `./gradlew :mobile:apps:customer-android:assembleDebug` — `BUILD SUCCESSFUL` locally, then confirmed green in real CI. Two full real `main` CI runs (round 1: `c673f26`, found the login/mobile fixes worked and surfaced findings 4 and 6 above; round 2: this commit, fixing both) — `repository`, `backend`, `contracts`, `web`, and `mobile` all genuinely green on GitHub's own runner, not just locally. `secrets-and-node-dependencies` and `infrastructure` not yet confirmed clean as of writing this entry (round 2 was still running) — see the round 2 evidence note appended below once it lands, or the next packet's own verification if this entry wasn't updated in time.
...
- **Rollback considerations:** every change here is CI config, ignore-file entries, a test-timeout config value, and documentation — no application code, schema, or runtime behavior changed. Safe to revert independently of WEGO-012 or any other packet.
