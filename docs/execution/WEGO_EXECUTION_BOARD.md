...
- **What CI's first real run found, and what was fixed:**
  ...
  3. **`secrets-and-node-dependencies` job — a real gitleaks false positive, on this repository's first-ever gitleaks scan.** Because this whole history had never been pushed, this was also the first time gitleaks ever actually scanned it. It flagged `generic-api-key` on a hardcoded test header value (`header("Idempotency-Key", "correlation-propagation-key-1")` in `CorrelationPropagationHttpTest.kt`, from the original WEGO-002 commit `82b3834`) — inspected directly and confirmed a synthetic test fixture string, never a real credential. **Fixed**: added `.gitleaksignore` at the repo root with that one finding's exact fingerprint (gitleaks's own documented mechanism), not a blanket rule change — every future finding still gets scanned and must be individually verified before being added there.
  4. **`infrastructure` job — one flaky failure in the pre-existing, untouched WEGO-002 booking test**, unrelated to anything WEGO-012/013 touched: `create offering` timed out waiting for `getByText(OFFERING_TITLE)` to appear after form submission. All 5 other lifecycle tests in the same run (HR, attendance/leave, accounting, payroll, financial reports — including everything WEGO-012 added) passed cleanly in the same run. Single occurrence, no local reproduction attempted. **Not fixed by this packet** — flagged as an open risk below rather than guessed at; needs either a reproduction under load or an accepted-flake note, not a blind timeout bump.
...
- **Affected files:** `.github/workflows/ci.yml` (ripgrep install step, mobile job's Gradle invocation), `.gitleaksignore` (new), `docs/execution/WEGO_EXECUTION_BOARD.md` (this entry, and the WEGO-011/WEGO-012 status corrections above), `docs/operations/CLIENT_ONBOARDING.md` (new).
- **Evidence:** `bash scripts/repository-check.sh` — clean, locally, after the status-string fixes (previously failed on the WEGO-011 row before this packet). `./gradlew :mobile:apps:customer-android:assembleDebug` — `BUILD SUCCESSFUL`, run standalone with the same `JAVA_HOME`/`ANDROID_HOME` this repo's mobile work has used throughout. The `.gitleaksignore` fingerprint was copied verbatim from the actual CI failure log's own `Fingerprint:` line, not hand-constructed — not independently re-run locally, since `gitleaks` isn't installed on this box; confirmed by the next real `main` CI run instead.
- **Open risks, explicitly not closed by this packet:**
  1. The `infrastructure` job's single Playwright flake (finding 4 above) — needs a real decision (reproduce and fix, or document as an accepted flake with a retry) once it's seen a second time or someone chooses to chase it deliberately.
  2. `gradle-dependency-submission` will keep failing every push to `main` until the repository owner enables Dependency graph in GitHub's own repository settings (finding 5 above) — not something a commit can fix.
- **Rollback considerations:** every change here is CI config, a new ignore file, and documentation — no application code, schema, or runtime behavior changed. Safe to revert independently of WEGO-012 or any other packet.
- **NEXT PACKET:** none queued.
