...
- **Affected files:** `.github/workflows/ci.yml` (ripgrep install step, mobile job's Gradle invocation), `.gitleaksignore` (new, 2 entries), `e2e/playwright.config.ts` (expect timeout), `infrastructure/nginx/nginx.conf` (login rate limiter burst), `docs/execution/WEGO_EXECUTION_BOARD.md` (this entry, and the WEGO-011/WEGO-012 status corrections above), `docs/operations/CLIENT_ONBOARDING.md` (new).
- **Evidence:** `bash scripts/repository-check.sh` — clean, locally. `./gradlew :mobile:apps:customer-android:assembleDebug` — `BUILD SUCCESSFUL` locally, then confirmed green in real CI. `nginx -t` against the edited `nginx.conf` (via the exact pinned image `compose.yaml` uses) parsed cleanly — reached upstream-hostname resolution (which only fails standalone, outside the real Compose network), confirming no syntax error in the burst change. Three full real `main` CI runs total (round 1: `c673f26` — found the rg/mission-table/gitleaks/mobile bugs, fixed 3 of 4; round 2: `4cfbfbe` — fixed the gitleaks self-quote repeat for real, misdiagnosed the E2E failure as timing jitter; round 3: this commit — the wrong diagnosis was caught by round 3's own rerun repeating the identical failure at the doubled timeout, root-caused to the nginx login-rate limiter, and fixed for real with the owner's explicit go-ahead). Round 3's own CI run (`06e2503`) confirmed the real fix: `repository`, `backend`, `contracts`, `web`, `mobile`, `secrets-and-node-dependencies`, and `infrastructure` — including its own "Verify login rate limiting" step, proving the loosened limiter still genuinely rate-limits — all green on GitHub's real runner. `gradle-dependency-submission` is the sole remaining red job, and it is expected to stay red until finding 5's GitHub repository setting is toggled (not something any commit can fix).
